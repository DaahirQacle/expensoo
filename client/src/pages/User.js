...
export default function User() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = expenses.map((n) => n.name);
    ...
    <Page title="User">
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Link to="/dashboard/app/add-expense" style={{ textDecoration: 'none' }}>
            {' '}
            <Button
              variant="contained"
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="left">
                          {' '}
                          <Label variant="ghost" color={(row.expense === 'expense' && 'error') || 'success'}>
                            {sentenceCase(row.expense)}
                          </Label>{' '}
                        </TableCell>
                        <TableCell align="left">{row.createdAt}</TableCell>

                        <TableCell align="right">
                          <UserMoreMenu id={row._id} expenses={expenses} />
                        </TableCell>
                      </TableRow>
                    );
                  })}
